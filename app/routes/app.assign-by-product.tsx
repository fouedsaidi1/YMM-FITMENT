// app/routes/app.assign-by-product.tsx
...
export default function AssignByProduct() {
  ...
  const toggleAll = () => {
    setSelectedModelIds(
      selectedModelIds.size === vehicles.length ? new Set() : new Set(vehicles.map(v => v.id))
    );
  };
  ...
  const vehicleRows = vehicles.map(v => [
    <Checkbox label="" checked={selectedModelIds.has(v.id)} onChange={() => toggleVehicle(v.id)} />,
  ...
  return (
    <Page title="Assign by Product" backAction={{ content: "Dashboard", url: "/app" }}>
      <Layout>
        {result && (
          <Layout.Section>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              <Text as="h2" variant="headingMd">Step 1 — Select Products</Text>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              <Text as="h2" variant="headingMd">Step 2 — Filter Vehicles</Text>
              ...
              {vehicles.length > 0 && (
                <BlockStack gap="300">
                  <InlineStack align="space-between">
                    <Text as="p" variant="bodySm" tone="subdued">{vehicles.length} vehicles found</Text>
                    <Button size="micro" onClick={toggleAll}>
                      {selectedModelIds.size === vehicles.length ? "Deselect All" : "Select All"}
                    </Button>
                  </InlineStack>
                  <DataTable
          ...
          <Card>
            <BlockStack gap="400">
              ...
              <Text as="p" variant="bodySm" tone="subdued">
                {selectedProducts.length} product(s) × {selectedModelIds.size} vehicle(s) = <strong>{selectedProducts.length * selectedModelIds.size} rules</strong> will be created.
              </Text>
              <Button
                variant="primary"
